...
(function () {
  ...
  if ("IntersectionObserver" in window && reveals.length) {
    ...
    // Failsafe: never leave content stuck hidden if IntersectionObserver
    // doesn't fire (background-tab loads, throttled callbacks, etc.).
    const revealAll = () => reveals.forEach((el) => el.classList.add("in"));
    window.addEventListener("load", () => setTimeout(revealAll, 1200), { once: true });
