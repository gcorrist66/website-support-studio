// Phase 6X-DB — Guarded dev verification of the full auth pipeline (Supabase DEV only).
//
// Temporarily links a seeded dev operator to a SYNTHETIC auth_user_id, builds a SYNTHETIC session
// (NOT a real Supabase Auth session), then runs the full pipeline
// (session → principal → adapter → operator session → capability flags) against the real dev rows,
// and clears the link. No real auth users, no login, no RLS; fully reversible.
...
const SYNTHETIC_AUTH_USER_ID = "00000000-0000-4000-8000-00000000f001";
...
function transpile() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wss-auth-pipeline-db-"));
...
async function main() {
  ...
  try {
    ...
    // Full pipeline: synthetic session → principal → adapter → operator session.
    const session = rt.createSyntheticSession({ id: SYNTHETIC_AUTH_USER_ID, email: "synthetic@wss-dev.test", expiresAtIso: FUTURE });
    const result = rt.resolveOperatorSessionFromSession(session, operatorRows, { nowIso: NOW });
    assert(result.authenticated === true && result.session !== null, `pipeline should authenticate the linked operator (reason: ${result.reason})`);
    assert(result.session.email === LINK_TARGET && result.session.role === "gary_approver", "pipeline resolved the linked gary_approver operator");
    mark("pipeline resolves dev session from synthetic session", true, `${LINK_TARGET} resolved through session→principal→adapter`);

    const flags = rt.resolveCapabilityFlagsFromSession(session, operatorRows, { nowIso: NOW });
    assert(flags.canSeeApproveReply === true && flags.canSeeRejectReply === true, "gary can approve/reject");
    assert(flags.canSeeCreateTicket === false && flags.canSeeSendReply === false, "gary cannot create/send");
    mark("capability flags correct", true, "gary approve/reject yes; create/send no");

    // No operator rows were mutated/linked by the pipeline.
    ...
    mark("pipeline wrote nothing", true, "resolution did not link/mutate any operator");
    ...
    mark("local auth-pipeline-db validation", true, "dev full-pipeline resolution verified; no real auth users; no RLS; cleaned up");
    console.log(JSON.stringify({ status: "pass", checks }, null, 2));
  } catch (error) {
    mark("local auth-pipeline-db validation", false, error instanceof Error ? error.message : String(error));
