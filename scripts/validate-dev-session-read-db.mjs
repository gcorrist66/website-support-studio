// Phase 7I-DB — Guarded dev verification of the dev session-read path (Supabase DEV only).
//
// Temporarily links a seeded dev operator to a SYNTHETIC auth_user_id, then runs the dev session-read
// path with a SYNTHETIC session against the real dev rows, verifies the OperatorSession + capability
// flags, and clears the link. No real auth users, no sign-in, no RLS; fully reversible.
...
const SYNTHETIC_AUTH_USER_ID = "00000000-0000-4000-8000-0000000a0201";
...
function transpile() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wss-dev-session-read-db-"));
...
async function main() {
  ...
  try {
    ...
    // Dev session-read path: synthetic session → principal → pipeline → operator session.
    const state = rt.createSyntheticSessionReadState(
      { id: SYNTHETIC_AUTH_USER_ID, email: "synthetic@wss-dev.test", expiresAtIso: FUTURE },
      operatorRows,
      { nowIso: NOW },
    );
    assert(state.principal && state.principal.id === SYNTHETIC_AUTH_USER_ID, "session read extracted the principal");
    assert(state.adapterResult && state.adapterResult.session && state.adapterResult.session.email === LINK_TARGET && state.adapterResult.session.role === "cs_agent", `session read should resolve ${LINK_TARGET} (reason: ${state.adapterResult?.reason})`);
    mark("dev session-read resolves dev operator session", true, `${LINK_TARGET} resolved via synthetic session read`);

    assert(state.capabilityFlags.canSeeCreateTicket === true && state.capabilityFlags.canSeeCloseTicket === true && state.capabilityFlags.canSeeApproveReply === false, "cs_agent capability flags correct");
    ...
    mark("session read wrote nothing", true, "read path did not link/mutate any operator");
    ...
    mark("local dev-session-read-db validation", true, "dev session-read path verified; no real auth users; no RLS; cleaned up");
    console.log(JSON.stringify({ status: "pass", checks }, null, 2));
  } catch (error) {
    mark("local dev-session-read-db validation", false, error instanceof Error ? error.message : String(error));
